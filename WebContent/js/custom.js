(function($) {
	...
	// Config
	// -------------------------------------------------------------------------------
	var companyName = 'Wellness-Sutra';
	var address = 'Glenarty, 40 Kasturba Road Cross, Bangalore 560001 '; // Enter
																			// your
																			// Address
			...
			.on(
					...
					function() {
						...
						$("#blogModal .modal-body")
								.html(
										"<div class='row'><div class='col-lg-4 col-lg-offset-4' ><img class='img-responsive' src='img/loading_spinner.gif'></div></div>");
						$('#blogModal').modal('show');
						$("#blogModal").load(service, function() { // calback
																	// function
						});
	...
	// Gallery
	// -------------------------------------------------------------
	$(".gallery .gallery-thumbnail-container").on(
			"click",
			function() {

				var src = $(this).find("img").data('img');
				var galleryImg = $('<img/>').attr('src', src).addClass(
						'img-responsive');
				...
				$('#galleryModal').on(
						'shown.bs.modal',
						function() {
							$('#galleryModal .modal-dialog').css('max-width',
									galleryImgWidth);
							$('#galleryModal .modal-body').html(galleryImg);
							$('#galleryModal .modal-nav .title').html(
	...
	/*
	 * fix vertical when not overflow call fullscreenFix() if .fullscreen
	 * content changes
	 */
				...
				$.ajax({
					...
					success : function(data) {
						...
						if ('success' == data.result) {
							$('#contact-form-gmap-msg').css('visibility',
									'visible').hide().fadeIn().removeClass(
									'hidden').addClass('alert-success');
							$('#contact-form-gmap-msg').html(data.msg[0]);
							$('#contact-form-gmap .btn-submit').removeAttr(
									'disabled');
	...
	$("#appointment-form")
			.submit(
					function() {
						...
						$('#appointment-form .btn-submit').attr('disabled',
								'disabled');
	...
	$("#newsletter-form").submit(
			function() {
				...
				$('#newsletter-form input[type=submit]').attr('disabled',
						'disabled');
				...
				$.ajax({
					...
					success : function(data) {
						...
						if ('success' == data.result) {
							$('#newsletter-form-msg').css('visibility',
									'visible').hide().fadeIn().removeClass(
									'hidden').addClass('alert-success');
	...
	var contentString = '<div id="content">' + '<strong>' + companyName
			+ '</strong><br>'
			+ 'Glenarty, 40 Kasturba Road Cross, Bangalore 560001 ' + '</div>';
